...
BOT_BUILD = "2026.08.06-night-shift-v2"
...
def db_save_daily(
    ...
    """Атомарно сохраняет завершённые сутки и последнюю первую смену.

    Пустые колонки после последнего дня с измерениями игнорируются. Нулевые дни
    между заполненными днями сохраняются. Ненулевые будущие даты отклоняются.
    Последний день без показаний второй смены считается незавершённым независимо
    от календарной даты сервера.
    """
    ...
    # Нулевой день между измеренными днями может быть реальной остановкой.
    # Нулевые колонки после последнего измеренного дня являются хвостом шаблона.
    last_measured_day, last_measured_date = max(measured_days, key=lambda item: item[1])
    data_days = [
        (day_num, report_day)
        for day_num, report_day in valid_days
        if report_day <= last_measured_date
    ]
    data_days.sort()
    by_day = {day_num: all_valid_by_day[day_num] for day_num, _report_day in data_days}
    max_day = last_measured_day
    latest_shifts = by_day[max_day]
    latest_shift_1 = latest_shifts.get(1, {})
    latest_shift_2 = latest_shifts.get(2, {})
    latest_has_shift_1 = _shift_has_measurements(latest_shift_1)
    latest_has_shift_2 = _shift_has_measurements(latest_shift_2)
    incomplete_day = max_day if latest_has_shift_1 and not latest_has_shift_2 else None
    completed_days = [
        day_num
        for day_num, report_day in data_days
        if report_day < last_measured_date or incomplete_day is None
    ]
    ...
    with db_connection() as conn:
        ...
        # Кнопка «Ночная смена» всегда показывает Смену 1 последнего дня,
        # если она есть. При наличии Смены 2 тот же день также входит в сутки.
        if latest_has_shift_1:
            night_record = {
                "year": year,
                "month": month,
                "day_num": max_day,
                "source": filename,
                **{field: finite_number(latest_shift_1.get(field)) for field in FIELDS},
            }
        ...
        row = conn.execute(
            ...
            SELECT night_shift.*,
                   EXISTS(
                       ...
                       WHERE daily_data.year=night_shift.year
                         AND daily_data.month=night_shift.month
                         AND daily_data.day_num=night_shift.day_num
                   ) AS included_in_daily
            FROM night_shift
            WHERE night_shift.year=? AND night_shift.month=?
            ORDER BY night_shift.day_num DESC
            LIMIT 1
            """,
            (year, month),
        ).fetchone()
...
async def cmd_version(msg: Message, state: FSMContext):
    """Показывает метку сборки для проверки факта обновления."""
    await state.clear()
    await msg.answer(
        f"🧩 Сборка бота: `{BOT_BUILD}`\n"
        "В этой сборке ночная смена берётся из Смены 1 "
        "последнего заполненного дня отчёта.",
        parse_mode="Markdown",
    )
...
async def handle_report(msg: Message, state: FSMContext):
    ...
    if saved == 0:
        if incomplete_day is not None and night_available:
            text = (
                f"🌙 Файл прочитан. У последнего дня {incomplete_day} отсутствуют "
                "показания Смены 2, поэтому день не включён в суточный и "
                "скользящий баланс. Смена 1 доступна по кнопке *🌙 Ночная смена*."
            )
        elif incomplete_day is not None:
            text = (
                f"🌙 Файл прочитан. Последний день {incomplete_day} не включён "
                "в суточный баланс: полный комплект смен отсутствует."
            )
        else:
    ...
    if incomplete_day is not None:
        if night_available:
            last_day_note = (
                ...
                f"баланса целиком (Смена 2 отсутствует в файле) — Смена 1 "
                f"доступна отдельно по кнопке"
            )
        else:
            last_day_note = (
                f"🌙 День {incomplete_day}: исключён из суточного и скользящего "
...
async def report_night_shift(msg: Message, state: FSMContext):
    ...
    included_in_daily = bool(ns.get("included_in_daily"))
    if included_in_daily:
        status_note = (
            "_Смена 1 (19:30–07:30); её данные уже включены в завершённые сутки._"
        )
        footer_note = "_Показана отдельно для посменного контроля._"
    else:
        status_note = (
            "_Смена 1 (19:30–07:30); Смена 2 в загруженном файле отсутствует._"
        )
        footer_note = "_День пока не включён в суточный и скользящий баланс целиком._"
...
async def h_help(msg: Message, state: FSMContext):
    ...
    await msg.answer(
        ...
        "🌙 Ночная смена — Смена 1 последнего дня; бот укажет, вошла ли она в сутки\n"
